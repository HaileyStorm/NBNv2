...
public sealed class WorkerNodeDiscoveryAndPlacementTests
{
    ...
    [Fact]
    public async Task PlacementPeerLatencyRequest_Reports_PeerRoundTripSamples()
    {
                    ...
                    new PlacementPeerLatencyRequest
                    {
                        ...
                        Peers =
                        {
                            new PlacementPeerTarget
                            {
                                ...
                                WorkerAddress = workerBPid.Address,
    ...
    public async Task PlacementLatencyEchoRequest_Returns_Ack()
    {
        ...
        var workerPid = root.SpawnNamed(
            Props.FromProducer(() => new WorkerNodeActor(Guid.NewGuid(), string.Empty)),
            $"worker-echo-{Guid.NewGuid():N}");

        var response = await root.RequestAsync<PlacementLatencyEchoAck>(
            workerPid,
    ...
    [Fact]
    public async Task PlacementAssignmentRequest_NonFileStoreUri_Reuses_NodeLocalCache_Across_Assignments()
    {
        var workerArtifactRoot = Path.Combine(Path.GetTempPath(), $"nbn-worker-remote-store-{Guid.NewGuid():N}");
        Directory.CreateDirectory(workerArtifactRoot);
        ...
        try
        {
            ...
            var richNbn = NbnTestVectors.CreateRichNbnVector().Bytes;
            var manifest = await remoteScope.Store.StoreAsync(new MemoryStream(richNbn), "application/x-nbn");
            var brainDef = manifest.ArtifactId.Bytes.ToArray()
            ...
            var firstAck = await harness.Root.RequestAsync<PlacementAssignmentAck>(
                ...
                {
                    Assignment = BuildAssignment(
                        assignmentId: "assign-remote-store-1",
            ...
            var secondAck = await harness.Root.RequestAsync<PlacementAssignmentAck>(
                ...
                {
                    Assignment = BuildAssignment(
                        assignmentId: "assign-remote-store-2",

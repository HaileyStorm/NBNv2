...
* Simulator applies deterministic plateau recovery pressure: if successful speciation-commit similarity minima stop improving over a sustained window, it temporarily increases inverse-compatibility run pressure (higher `run_count`) and relaxes back when commit minima improve.

...
- Random brain generation applies activity guardrails in non-fixed modes: input-region neurons are normalized to a signal-responsive profile (`ACT_IDENTITY`, `RESET_ZERO`, `ACCUM_SUM`, zero thresholds), output-region neurons use `RESET_ZERO` with a bounded random activation subset (`ACT_CLAMP`, `ACT_TANH`, `ACT_SIG`), `ACCUM_NONE` is excluded from random weighted picks, and random threshold ranges are capped to avoid dead-on-arrival or runaway defaults.
...
- Buffer monitoring semantics are tick-continuous while visualization is active for the monitored scope (focused region or full-brain): `VizNeuronBuffer` streams current per-neuron buffer values each tick, including zeros, so charts/debug views do not stall during low-activity windows.

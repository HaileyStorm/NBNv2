...
- Mirror service endpoint settings from SettingsMonitor (`service.endpoint.hivemind`, `service.endpoint.io_gateway`) into Orchestrator endpoint inputs at refresh/startup, while preserving deterministic fallback to live node-derived HiveMind host/port when those settings are absent or invalid.

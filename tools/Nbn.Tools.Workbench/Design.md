...
- Mirror service endpoint settings from SettingsMonitor (`service.endpoint.hivemind`, `service.endpoint.io_gateway`, `service.endpoint.reproduction_manager`, `service.endpoint.speciation_manager`, `service.endpoint.worker_node`, `service.endpoint.observability`) into Orchestrator endpoint inputs at refresh/startup and live setting updates, while preserving deterministic fallback to live node-derived HiveMind host/port when those settings are absent or invalid.
...
- Local Launch uses SettingsMonitor as the only required cross-service bootstrap coordinate for non-Settings services; Workbench does not inject static peer host/port wiring between runtime services.

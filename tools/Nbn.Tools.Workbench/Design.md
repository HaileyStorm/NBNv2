...
- Speciation pane visualizes taxonomy dynamics with two live-updating history charts derived from `SpeciationListHistory`: a per-species population line chart across epochs and a top-to-bottom stacked flow chart where horizontal width denotes per-epoch relative population share (0-100%).

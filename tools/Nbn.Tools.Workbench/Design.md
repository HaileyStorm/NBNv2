...
- Random brain generation applies activity guardrails in non-fixed modes: input-region neurons are normalized to a signal-responsive profile (`ACT_IDENTITY`, `RESET_ZERO`, `ACCUM_SUM`, zero thresholds), output-region neurons use `RESET_ZERO`, `ACCUM_NONE` is excluded from random weighted picks, and random threshold ranges are capped to avoid dead-on-arrival or runaway defaults.

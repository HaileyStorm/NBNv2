...
- Run real CPU and GPU runtime rows in `localhost-stress` when current worker capability telemetry reports effective ILGPU runtime capacity after CLI/resource limits and VRAM headroom are applied; skip GPU rows when compatible hardware is temporarily CPU-only because accelerator creation, GPU score, GPU compute quota, or GPU VRAM quota/headroom is unavailable.
- Keep the runtime GPU-threshold defaults anchored to measured localhost harness data. The current `auto` baseline comes from the compute-dominant recurrent localhost harness and is treated as `262144` neurons at GPU score `550.258`, then scaled against each worker's effective GPU score at runtime/planning time.
- Treat completed localhost-stress measurements as report data, not hard pass/fail performance gates for a specific machine. Rows fail only when runtime execution or backend verification fails for non-transient reasons; GPU backend verification that falls back because a compatible accelerator has no current runtime capacity is reported as a skipped GPU row with the runtime fallback reason preserved. Sub-target results remain passed measurements with the observed ceiling in the metrics/summary.

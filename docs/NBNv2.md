...
* For `brain_id`, `artifact_ref`, or `artifact_uri` candidates without explicit `species_id`, runtime assignment derives species deterministically from parent membership evidence (`parents`) plus optional similarity metrics in `decision_metadata_json`. Decision similarity prefers dominant-species pairwise evidence (`lineage.parent_a_similarity_score` / `lineage.parent_b_similarity_score`) when available, then falls back to lineage/global similarity fields.

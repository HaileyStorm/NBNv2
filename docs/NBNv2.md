...
- `SpeciationListHistoryRequest.limit` is runtime-clamped to a bounded page size; clients should use `total_records` plus `offset` to fetch additional pages and must not assume one request can retrieve an unbounded history.

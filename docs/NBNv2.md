...
* Assignment provenance is recorded in `SpeciationDecision.decision_metadata_json` (strategy, policy snapshot, lineage inputs, parsed scores, assigned-species similarity, source/dominant-species similarity, optional `lineage.intra_species_similarity_sample`, dynamic split-threshold source/values, and split-proximity deltas). Split founders still record `lineage.intra_species_similarity_sample = 1.0` as singleton cohesion metadata, but that synthetic founder score does not seed the dynamic species floor or satisfy the newborn re-split guard; the floor begins on the first non-founder assigned-species sample. Runtime emits assigned-species policy/dynamic thresholds under the generic `lineage_policy_effective_split_threshold` / `lineage_dynamic_split_threshold` keys and preserves source-species split pressure under explicit source-prefixed threshold/proximity keys so operator tooling can distinguish newborn cohesion from source-lineage split pressure deterministically.

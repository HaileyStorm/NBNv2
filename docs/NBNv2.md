...
* Membership remains immutable for external callers within an epoch; changing species still requires new-epoch flow (`SpeciationSetConfig.start_new_epoch=true` or reset). Runtime may apply an internal bounded hindsight reassignment immediately after a split to reclassify recent source-species memberships when policy predicates match.

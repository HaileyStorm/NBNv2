...
* Owns PPO reward-policy run-control and per-candidate reproduction-action artifact rollout orchestration outside the tick/shard runtime
* Modulates enabled reproduction mutation knobs from evaluated reward feedback; it does not backprop-train live RegionHost brain execution
...
For workers, these values are expected to come from real host probing or explicit operator configuration, not placeholder defaults. CPU/GPU scores come from a cached representative high-load RegionShard microbenchmark so placement chips and placement scoring reflect backend-relevant work instead of synthetic vector math. Worker CLI/resource limits are part of the effective capability snapshot: a GPU compute or GPU VRAM limit of zero makes the worker CPU-only for placement even if compatible GPU hardware is physically visible. Likewise, a compatible GPU whose accelerator cannot currently be created or whose VRAM cannot support the runtime benchmark is published as CPU-capable but not GPU-capable until a later capability refresh succeeds. HiveMind owns the settings-backed worker capability rerun cadence (`worker.capability.benchmark_refresh_seconds`) by sending refresh requests to workers; workers publish the refreshed result back through the normal SettingsMonitor heartbeat path instead of bypassing the stored snapshot. Routine placement churn does not force an immediate re-benchmark; placement pressure decisions use the live load/pressure fields already present in the regular worker heartbeats.
...
* incompatible or currently unusable ILGPU accelerators are filtered out during capability probing so worker telemetry, tests, perf probes, and runtime dispatch all agree before shard execution starts; transient VRAM exhaustion leaves the node available for CPU placement until a later capability refresh restores GPU availability
...
Direct reward-control controller state is not implicitly part of a brain snapshot. The direct runtime reward-control ledger is ephemeral HiveMind admission state. Supported reward-control surfaces update live runtime config: homeostasis fields follow the existing `.nbs` homeostasis-config persistence path when snapshots carry that block, while plasticity/cost/output-mode controls remain reconstructable live configuration rather than hidden `.nbs` controller data. Future surfaces must classify each controlled value as persisted in `.nbs`, ephemeral runtime state, or reconstructable controller configuration before that value may participate in recovery semantics. Reward observations for controller updates must refer to a completed tick boundary or an explicit observation fence, not an opportunistic live read.
...
* Dedicated compatibility assessment requests support both addressing modes and return similarity metrics without child synthesis/spawn; speciation uses these assessment-only calls when deciding whether a candidate may join an already-created derived species and when bootstrapping the first three non-founder members of a newborn derived species. Runtime handles reproduction and assessment requests re-entrantly so these speciation checks do not starve behind unrelated reproduction work, and repeated artifact-based assessments reuse a bounded parsed-parent cache keyed by artifact SHA.
...
Reward-policy optimization over reproduction parameters is artifact-oriented. The optional PPO optimizer may modulate enabled reproduction mutation probabilities and submit child artifact requests, but it does not own live Brain runtime-control actions. Direct reward-control over a running Brain is the IO/HiveMind contract in Section 13.6 and must not tunnel structural mutation through live state controls.
...
* Speciation bootstrap admissions use these assessment-only calls with a short bounded timeout and require the assigned-species score to clear the assigned species' current dynamic split threshold before a newborn derived species may seed its in-species floor. Bootstrap seeding still compares the assigned-species score against the pre-split source species, but it treats near-ties within the configured split-guard margin as sufficient evidence for the newborn lineage instead of demanding a strict source-species win on every early admission. Those assessment-only requests reuse the active reproduction compatibility thresholds/settings while forcing `SpawnChildNever`, so speciation sees the same compatibility gate semantics as reproduction without synthesizing or spawning a child. During the newborn bootstrap window, speciation evaluates admission against one fixed earliest exemplar from the target species instead of taking a rolling minimum across every early member, so the workload and gate do not ratchet upward while a species is still trying to grow from one founder toward its first three non-founder samples. Speciation owns the final admit/reject decision from those checks: if reproduction returns a finite manual similarity score, speciation may use that score even when `SimilarityReport.compatible=false` so long as the abort reason reflects a real similarity mismatch rather than lookup/load/runtime failure. Founder hindsight batches reuse one fixed earliest-exemplar snapshot per newborn species so bounded reassignment does not recursively amplify compatibility workload inside the same split commit, and founder commits enqueue that hindsight work after the founder assignment instead of blocking the originating commit response.
...
* keep a node-local cache of identity-verified full artifacts/chunks so repeated loads reuse local bytes after the first fetch
...
* fully stage built-in HTTP payloads and verify the requested manifest identity, exact byte length, and full-payload SHA-256 before exposing bytes or persisting them in the node-local cache
...
* the service base URI must expose:
  ...
  * `GET {base}/v1/artifacts/{sha256}` with `Range: bytes=start-end` -> exact partial bytes when supported for compatible clients
...
* the built-in client derives range views locally from fully verified canonical bytes; it does not trust a partial response that cannot prove the requested whole-artifact identity
...
* the built-in HTTP(S) client downloads and verifies the full canonical payload before deriving an indexed range view locally, because an untrusted partial response alone cannot prove the requested whole-artifact SHA-256; other cryptographically verified adapters may still implement selective remote fetches
...
* built-in HTTP(S) backends use the same node-local cache path (`NBN_ARTIFACT_CACHE_ROOT` or `<artifact-root>/.cache`) after the first successful identity-verified remote fetch/write-through; cache contents are never authoritative over upstream manifest/content responses
...
message SynchronizeBrainRuntimeConfig {
  nbn.Uuid brain_id = 1;
}
...
enum DirectRuntimeRewardControlSurface {
  ...
  DIRECT_RUNTIME_REWARD_CONTROL_SURFACE_HOMEOSTASIS_BASE_PROBABILITY = 2;
  DIRECT_RUNTIME_REWARD_CONTROL_SURFACE_COST_ENERGY_ENABLED = 3;
  DIRECT_RUNTIME_REWARD_CONTROL_SURFACE_PLASTICITY_ENABLED = 4;
  DIRECT_RUNTIME_REWARD_CONTROL_SURFACE_PLASTICITY_PROBABILISTIC_UPDATES = 5;
  DIRECT_RUNTIME_REWARD_CONTROL_SURFACE_PLASTICITY_DELTA = 6;
  DIRECT_RUNTIME_REWARD_CONTROL_SURFACE_PLASTICITY_REBASE_THRESHOLD_PCT = 7;
  DIRECT_RUNTIME_REWARD_CONTROL_SURFACE_HOMEOSTASIS_ENABLED = 8;
  DIRECT_RUNTIME_REWARD_CONTROL_SURFACE_HOMEOSTASIS_ENERGY_COUPLING_ENABLED = 9;
  DIRECT_RUNTIME_REWARD_CONTROL_SURFACE_HOMEOSTASIS_ENERGY_TARGET_SCALE = 10;
  DIRECT_RUNTIME_REWARD_CONTROL_SURFACE_HOMEOSTASIS_ENERGY_PROBABILITY_SCALE = 11;
  DIRECT_RUNTIME_REWARD_CONTROL_SURFACE_OUTPUT_VECTOR_SOURCE = 12;
}
...
message PlacementWorkerExclusionCount {
  string reason_code = 1;
  uint32 count = 2;
}

message PlacementWorkerExclusionDiagnostic {
  nbn.Uuid worker_node_id = 1;
  string worker_address = 2;
  string worker_root_actor_name = 3;
...
message PlacementWorkerInventory {
  ...
  fixed64 snapshot_ms = 2;
  uint32 total_workers_seen = 3;
  repeated PlacementWorkerExclusionCount exclusion_counts = 4;
  repeated PlacementWorkerExclusionDiagnostic excluded_workers = 5;
}
...
message RegisterIoGateway {
  ...
  optional nbn.control.InputCoordinatorMode input_coordinator_mode = 3;
  optional bool input_tick_drain_armed = 4;
}
...
- `SpeciationListHistoryRequest.limit` is runtime-clamped to a bounded page size; clients should use `total_records` plus `offset` to fetch additional pages and must not assume one request can retrieve an unbounded history. `SpeciationListMemberships` resolves an omitted epoch to the current epoch, is capped for operator safety, and should be treated as a bounded current-scope summary, not as the bulk history export path.
...
- The direct brain/runtime reward-control safety model has IO/HiveMind/Workbench API coverage for paused bounded plasticity-rate control plus barrier-queued bounded runtime-config surfaces; structural mutation and arbitrary neuron state control remain future work.

...
* For `brain_id`, `artifact_ref`, or `artifact_uri` candidates without explicit `species_id`, runtime assignment derives species deterministically from parent membership evidence (`parents`) plus optional similarity metrics in `decision_metadata_json`. Runtime keeps source/dominant-species similarity and assigned-species similarity distinct: source/dominant pairwise evidence drives split/hindsight logic, while assigned-species pairwise evidence drives intra-species cohesion and may rescue low-consensus mixed-parent assignments when it meets `lineage_match_threshold`.
...
* Assignment provenance is recorded in `SpeciationDecision.decision_metadata_json` (strategy, policy snapshot, lineage inputs, parsed scores, assigned-species similarity, source/dominant-species similarity, optional `lineage.intra_species_similarity_sample`, dynamic split-threshold source/values, and split-proximity deltas). Split founders seed `lineage.intra_species_similarity_sample = 1.0`, so a new species starts as a maximally cohesive singleton until accepted in-species variety lowers that floor. Runtime emits both policy and dynamic effective thresholds (`lineage_policy_effective_split_threshold`, `lineage_dynamic_split_threshold`) so operator tooling can interpret species-floor-driven split decisions deterministically.
* Membership remains immutable for external callers within an epoch; changing species still requires new-epoch flow (`SpeciationSetConfig.start_new_epoch=true` or reset). Runtime may apply an internal bounded founder-similarity-band reuse around a split: recent source-species members may be reassigned immediately after the split, and nearby post-split source members may reuse the same derived species when their source-species similarity remains within `founder_similarity +/- lineage_hindsight_similarity_margin`.

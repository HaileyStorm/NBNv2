...
VARIANTS = [
    Variant(
        slug="bridge-soft-narrow-ink",
        title="Soft + Narrow Ink",
        kind="loop",
        tweak="fixed-width baseline",
        note="Keeps the chosen narrower B width, but returns the whole wordmark to ink so color is the only remaining variable.",
        b_width_mult=0.80,
        stroke_mult=0.98,
    ),
    Variant(
        slug="bridge-soft-gold-b",
    ...
    Variant(
        slug="bridge-soft-gold-right-n-only",
        title="Soft + Gold Right N",
        kind="loop",
        tweak="gold right N only",
        note="Leaves the left N and the narrow B in ink, with only the terminal right N carrying the warm accent.",
        b_width_mult=0.80,
        stroke_mult=0.98,
        n2_color="#E0A31A",
    ),
    Variant(
        slug="bridge-soft-gold-b-gold-n",
...
def build_board() -> None:
    board = Image.new("RGBA", (1900, 1680), "#F5F0E8")
    ...
    draw.text((84, 44), "Bridge Soft fixed-width text study", fill="#17212D", font=title_font)
    draw.text((86, 112), "All loop variants use the same narrower B width. This pass isolates only wordmark color decisions, including a gold right-N-only option.", fill="#56636F", font=body_font)
    ...
    board.save(PNG_DIR / "nbn-round10-board.png")


def build_shortlist() -> None:
    shortlist = [
        "bridge-soft-narrow-ink",
        "bridge-soft-gold-b",
        "bridge-soft-gold-right-n-only",
    ...
    draw.text((96, 56), "Round 10 shortlist", fill="#17212D", font=title_font)
    draw.text((98, 124), "The clearest fixed-width loop options are the ink baseline, gold B, and gold right N, with Diamond Wide Gate as the non-loop reference.", fill="#56636F", font=body_font)
    ...
    board.save(PNG_DIR / "nbn-round10-shortlist.png")

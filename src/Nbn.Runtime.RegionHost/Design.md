...
Placement-managed RegionHost launches must receive the active placement epoch through `--placement-epoch` or `NBN_REGIONHOST_PLACEMENT_EPOCH`. The same epoch is forwarded on shard registration and unregistration so HiveMind can ignore stale lifecycle messages from previous placement executions.

...
- PPO rollout execution observes parent brains through IO `BrainInfoRequest` plus `RequestSnapshot` responses that are marked `generated_from_live_state`, applies the current controller policy to reproduction mutation probabilities, then submits `ReproduceByArtifactsRequest` to Reproduction with `SpawnChildNever` and IO-region count protection enabled.
...
- External evaluators can return `PpoRecordRewardsRequest` samples through IO after candidates are evaluated. PPO validates finite rewards, computes clipped advantage updates from recent candidate rewards, and updates future reproduction-action policy state. This is controller-level reward optimization; NBN brain execution itself is not backprop-trained.

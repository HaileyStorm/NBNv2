...
Remote adapters preserve content-addressed reads/writes by hash and may populate a node-local full-artifact cache after the first fetch. `IArtifactStore.TryOpenArtifactAsync(...)` continues to mean a complete artifact stream; selective reads use the separate `TryOpenArtifactRangeAsync(...)` path. The built-in HTTP client fully stages each remote payload, verifies its manifest length and SHA-256 against the requested artifact identity, and only then exposes it or allows a node-local cache to persist it. HTTP range views are derived from that verified payload because an untrusted partial response alone cannot prove the whole-artifact identity. `.nbn` manifests may carry region-index metadata so other range-capable adapters can fetch only the required region section. Seekable `.nbn` writes auto-populate that index unless the caller supplies explicit `ArtifactStoreWriteOptions.RegionIndex` values. That auto-indexing path is best-effort: malformed or out-of-range `.nbn` header metadata skips persisted region-index entries instead of failing storage, and the artifact store still relies on downstream consumers for full format validation.

The built-in HTTP(S) backend expects `{base}/v1/manifests/{sha256}`, `{base}/v1/artifacts/{sha256}`, and `POST {base}/v1/artifacts` with artifact media type in `Content-Type`; the reachable publisher also accepts HTTP `Range` on artifact GETs for compatible clients. Optional region-index metadata is carried in `X-Nbn-Artifact-Region-Index` as a base64-encoded JSON array. Resolver-wrapped HTTP stores keep `enableNodeLocalCache=true` by default, so the first successful, identity-verified remote fetch/write-through hydrates the node-local cache under `NBN_ARTIFACT_CACHE_ROOT` (or `<artifact-root>/.cache`) without changing artifact identity. Resolver/store instance caches are concurrency-safe for parallel region-shard loads on one node, and when callers leave both `NBN_ARTIFACT_ROOT` and `NBN_ARTIFACT_CACHE_ROOT` unset the default local root is the standard user-local app-data path (`.../NBN/artifacts`) rather than the process working directory, so installed binaries do not depend on writable install locations. If the node-local cache path is invalid or unwritable, remote reads fall back to direct upstream access instead of failing artifact-backed runtime correctness.

Failure semantics are explicit: unresolved non-file URIs throw instead of falling back to local paths, unsupported env-map target types throw configuration errors, remote `404` responses mean the manifest/artifact is absent in that store, and other non-success transport responses bubble back to the runtime caller.

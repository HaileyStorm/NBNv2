...
- SQLite persistence keeps membership sticky within an epoch and retains historical epochs after reset. By default assignments are immutable; a bounded manager-only hindsight path may reassign recent source-species members to a just-created derived species when strict policy predicates pass.

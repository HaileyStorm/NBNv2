...
- Enforces contract-level dry-run versus commit semantics: dry-run never mutates state; commit requires `brain_id` candidates and preserves per-epoch membership immutability.
...
- Derives assignment outcomes from parent membership evidence plus optional similarity metrics in `decision_metadata_json` (for example `similarity_score` from reproduction reports), with deterministic tie-breaking and hysteresis-band hold behavior.
...
- Ingests lineage edges atomically on committed membership creation when parent `brain_id` references are provided, so lineage history and membership decisions stay in sync.

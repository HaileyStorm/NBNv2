...
- Split classification now uses a dynamic effective split threshold: `max(policy_effective_split_threshold, species_floor_similarity - lineage_hysteresis_margin)`, where `species_floor_similarity` is the persisted minimum full-consensus intra-species lineage similarity sample for the dominant species (`assigned species == dominant species` and `dominant_species_share == 1.0`).
- New derived species creation does not seed its own floor from the divergence event; floor samples are collected only from later full-consensus in-species assignments so new species start with high internal similarity and relax downward only as accepted in-species variety grows.
...
- Persists enriched decision provenance (`assignment_strategy`, policy snapshot, lineage inputs, parsed scores, lineage-similarity evidence, dynamic/policy split thresholds, split-threshold source, and split proximity deltas) in `decision_metadata_json` for evaluate/assign/batch flows.

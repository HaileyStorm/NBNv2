...
- Runtime default lineage policy is intentionally conservative against premature merge/re-split churn: `lineage_match_threshold=0.90`, `lineage_hysteresis_margin=0.03` (implied default split `0.87`), `lineage_split_guard_margin=0.02` (effective split `0.85`), `lineage_min_parent_memberships_before_split=1`, and `parent_consensus_threshold=0.70`.
...
- Persists enriched decision provenance (`assignment_strategy`, policy snapshot, lineage inputs, parsed scores, and lineage-similarity evidence such as `lineage.lineage_similarity_score` when supplied) in `decision_metadata_json` for evaluate/assign/batch flows.

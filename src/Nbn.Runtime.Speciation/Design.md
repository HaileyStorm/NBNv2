...
- Derives assignment outcomes from parent membership evidence plus optional similarity metrics in `decision_metadata_json`, keeping source-species similarity and assigned-species similarity distinct: dominant/source-species pairwise evidence drives split/hindsight decisions, while assigned-species pairwise evidence drives intra-species cohesion and low-consensus mixed-parent rescue when a non-empty parent species meets `lineage_match_threshold`.
...
- When parent consensus is below threshold, runtime may still assign into the strongest existing parent species if parent-specific pairwise similarity meets `lineage_match_threshold`, which prevents early mixed-parent descendants from defaulting away from a nearby newborn species that they strongly match.
...
- Persists enriched decision provenance (`assignment_strategy`, policy snapshot, lineage inputs, parsed scores, assigned-species similarity, source/dominant-species similarity, intra-species sample evidence, dynamic/policy split thresholds, split-threshold source, and split proximity deltas) in `decision_metadata_json` for evaluate/assign/batch flows.

...
try
{
    ...
    {
        Console.WriteLine("[WARN] Speciation endpoint discovery is disabled because SettingsMonitor coordinates were not configured.");
    }
    else
    {
        discoveryClient.EndpointObserved += observation =>
            system.Root.Send(managerPid, new SpeciationManagerActor.EndpointStateObserved(observation));
...
{
    Console.WriteLine($"[WARN] Speciation endpoint discovery setup failed: {ex.GetBaseException().Message}");
}
    ...
    CancellationToken cancellationToken)
{
    ...
    while (!cancellationToken.IsCancellationRequested)
    {
        try
        {
            ...
            system.Root.Send(managerPid, new SpeciationManagerActor.DiscoverySnapshotApplied(known));
        ...
        catch (Exception ex)
        {
            ...
            Console.WriteLine(
                $"[WARN] Speciation discovery transition target=discovery outcome=retry_scheduled failure_reason={failureReason} attempt={attempt} retry_ms={(long)retryDelay.TotalMilliseconds}: {ex.GetBaseException().Message}");

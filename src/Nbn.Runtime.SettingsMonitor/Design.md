...
- Persists per-brain update timestamps and periodically prunes dead brain/controller rows after a bounded retention window (currently six hours). Non-dead rows whose last brain update and controller heartbeat are both stale are also pruned after a longer retention window (currently twenty-four hours) so historical non-live rows do not accumulate without bound; freshness-aware consumers still layer their own live/recent filtering on top of the stored snapshot.

...
- Persists per-brain update timestamps and periodically prunes dead brain/controller rows after a bounded retention window (currently six hours) so operator queries do not grow without bound from historical dead entries; non-dead brain rows are never removed by that cleanup path.

...
- SQLite connections bound WAL growth with `wal_autocheckpoint`/`journal_size_limit`. Startup performs best-effort checkpointing only; larger free-page compaction runs with periodic stale-row pruning so service availability does not wait on `VACUUM`.

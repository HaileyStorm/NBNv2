...
- Treats stale HiveMind service-root failures such as `no longer exists` as recoverable discovery evidence: IO refreshes the canonical HiveMind endpoint from SettingsMonitor and retries the forwarding request once before returning a stable failure response.

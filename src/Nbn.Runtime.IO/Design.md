...
- Treats stale or missing optional service roots as recoverable discovery evidence where safe: IO refreshes canonical HiveMind and PPO manager endpoints from SettingsMonitor before returning stable unavailable/failure responses, and retries HiveMind/PPO forwarding once after stale-root errors such as `no longer exists`.

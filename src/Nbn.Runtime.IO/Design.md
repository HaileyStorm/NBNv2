...
- Exposes canonical IO/proto wrappers for external runtime operations, including spawn/kill lifecycle forwarding, pause/resume brain control forwarding, direct runtime reward-control forwarding to HiveMind for paused and barrier-queued runtime-config surfaces, separate queued-spawn and placed-and-visible wait acknowledgements, reproduction/speciation/PPO contract calls, global default and per-brain output-vector-source selection, barrier-coordinated whole-brain runtime state reset, and placement-worker capacity snapshots forwarded from HiveMind.
...
- Treats stale or missing optional service roots as recoverable discovery evidence where safe: IO refreshes canonical HiveMind, Speciation manager, and PPO manager endpoints from SettingsMonitor before returning stable unavailable/failure responses, and retries HiveMind/Speciation/PPO forwarding once after stale-root errors such as `no longer exists`.
- Forwards speciation stateful and batch requests to the dedicated Speciation manager endpoint discovered through SettingsMonitor without embedding assignment policy in the gateway.
- Forwards PPO status/start/stop requests to the dedicated PPO manager endpoint discovered through SettingsMonitor; external clients should keep PPO control traffic on the IO Gateway surface instead of targeting the PPO actor directly.
...
- Resolves missing `BrainInfo` entries through HiveMind metadata bootstrap with a tolerance for moderately slow metadata replies, while still falling back to a default empty `BrainInfo` when HiveMind does not answer in time. `AwaitSpawnPlacementViaIO` preserves HiveMind's placed/ready result as the placement source of truth, separately attempts metadata visibility within the caller's remaining placement budget, and warms the gateway's brain/coordinator cache when that metadata is visible.

...
- WorkerNode auto backend selection is conservative on Windows: `RegionShardComputeBackendPreference.Auto` defaults to CPU there unless the operator explicitly overrides the backend preference, so local/distributed placement stays functional even when ILGPU/GPU execution is available but not yet stable enough for unattended auto-selection.
